...
const AllBlogs = () => {
  ...
  const getFirst15Words = (content) => {
    if (!content) return "";
    const text = content.replace(/<[^>]*>/g, ""); // Basic HTML strip
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-[30px] gap-y-10">
            {Array.isArray(posts) && posts.map((post) => {
              ...
              const authorName = post.author || "Summit Home";
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <h3
                      className="text-[18px] font-bold mt-3 text-gray-900 leading-snug group-hover:text-[#941007] transition-colors duration-300 line-clamp-2 min-h-[54px]"
                    >
                      {post.title}
                    </h3>

                    <p className="text-[14px] text-gray-600 my-4 line-clamp-3 leading-relaxed">
                      {getFirst15Words(post.content)}
                    </p>

                    <div className="mt-auto flex items-center text-[14px] font-bold text-gray-900 group-hover:text-[#941007] transition-all duration-300">
                      <span className="relative">
                        Read Article
                        <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#941007] transition-all duration-300 group-hover:w-full"></span>
                      </span>
