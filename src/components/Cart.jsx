...
const Cart = () => {
  ...
  const getProductDetails = (productId) => {
    const raw = productCache[productId];

    if (!raw) {
      return { name: "Loading...", image: "/asset/images/dummy-image-square.webp", sku: null };
    }

    // API may return single product object or array (e.g. from view?search=)
    const product = Array.isArray(raw) ? raw[0] : raw;
    if (!product) {
      return { name: "Product", image: "/asset/images/dummy-image-square.webp", sku: null };
    }

    const name = product?.name || product?.product_name || product?.title || "Product";
    const image = getImage(product);
    const sku = product?.sku || product?.detail_sku || (product?.variants?.[0]?.sku) || null;

    return { name, image, sku };
  };
                    ...
                    <div className="md:hidden space-y-4">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="relative w-20 h-20 bg-gray-200 rounded flex-shrink-0 overflow-hidden">
                          ...
                          <img
                            src={imageErrors[item?.product_id] ? "/asset/images/dummy-image-square.webp" : getProductDetails(item?.product_id)?.image}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            SKU: {item?.variant?.sku || item?.sku || getProductDetails(item?.product_id)?.sku || "N/A"}
                          </p>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 sticky top-20 lg:top-28">
                ...
                <Link to="/" className="block mt-3">
                  <button className="w-full bg-gray-200 hover:bg-gray-300 text-gray-900 py-3 rounded-lg font-bold text-base sm:text-lg transition-colors">
                    Continue Shopping
                  </button>
                </Link>
              </div>
