...
const GasStoveSection = ({ user }) => {
  ...
  const imageURL = import.meta.env.VITE_APP_IMAGE_BASE_URL ?? 'https://api.summithomeappliance.com';
        ...
        {product?.data?.slice(0, 4).map((product, i) => (
          ...
          >
            ...
            <div className="relative w-full overflow-hidden rounded-2xl shadow-md">
                ...
                <img
                  src={
                    product?.variants?.[0]?.image
                      ...
                      : "/asset/images/dummy-image-square.webp"
                  }
                  alt={product.name}
              ...
              >
                <FontAwesomeIcon
                  icon={isInWishlist(product.id || product.product_id || product.sno || product.detail_id) ? solidHeart : regularHeart}
                  className={isInWishlist(product.id || product.product_id || product.sno || product.detail_id) ? "text-red-600" : "text-gray-400"}
                  style={{ fontSize: "14px" }}
                />
              </button>

              <span className="absolute bottom-2 left-2 bg-[#941007] text-white text-xs px-2 py-1 rounded">
                Sale
              </span>
            </div>
            ...
            <div className="text-center mt-4">
              ...
              <p className="text-sm text-gray-700 mb-3">
                from{" "}
                <span className="font-semibold text-[#941007]">
            ...
            <div className="flex gap-3 justify-center mt-auto">
              <button
                ...
                className="bg-[#941007] text-white text-sm px-4 py-2 rounded-full hover:bg-[#941007] transition"
              >
                Add to Cart
              </button>
              <button
                onClick={() => handleBuyNow(product?.variants?.[0]?.id)}
                className="text-[#941007] border border-[#941007] text-sm px-4 py-2 rounded-full hover:bg-[#941007] hover:text-white transition"
              >
                Buy Now
              </button>
            </div>
