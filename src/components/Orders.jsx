...
const Orders = () => {
  ...
  if (msg)
    return <p className="text-center text-[#941007] mt-10">{msg}</p>;
          ...
          >
            <div className="mb-4">
              ...
              <p className="text-sm">
                ...
                <span
                  className={`inline-block px-3 py-1 rounded-full text-white text-xs ${order.status === "pending"
                      ? "bg-[#941007]"
                ...
                >
                  {item.image_url ? (
                    <img
                      src={
                        item.image_url
                          ? `${import.meta.env.VITE_APP_IMAGE_BASE_URL ?? 'https://api.summithomeappliance.com'}/php_admin_panel/${item.image_url}`
